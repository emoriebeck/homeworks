...
The weakest relationship between between teacher and peer popularity ratings compared to expectation occurred in classroom $`r s.teach$class[1]`$, $\gamma_{10} + u_{1~97} = `r raneffs$popteach_c[1]`$.  
...
pred_pop <- pred_pop %>% 
  data.frame() %>% 
  setNames("B0") %>% 
  mutate(class = as.character(1:100)) %>% 
  full_join(ranef(mod2)[[1]]["popteach_c"] %>% 
              mutate(class = rownames(.))) %>%
  arrange(desc(popteach_c))
...
Coef <- bind_cols(Coef, By_School) %>% tbl_df %>%
  setNames(c("EB_Intercept","EB_Slope","Intercept_Var","Covariance","Covariance_Too",
                 "Slope_Var","Intercept_Res","Slope_Res","School","OLS_Intercept")) %>%
