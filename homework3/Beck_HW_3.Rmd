...
mod.pt1 <- lmer(popteach ~ 1 + gmc_texp + gmc_mean_pop_t + (1 | class), data = dat) 
...
```{r}
raneffs <- (fixef(mod1) + ranef(mod1)[[1]]) %>% 
  data.frame() %>% 
  mutate(class = rownames(.)) %>%
  arrange(desc(popteach_c))
```

The strongest relationship between between teacher and peer popularity ratings occurred in classroom $`r raneffs$class[1]`$, $\gamma_{10} + u_{1~97} = `r raneffs$popteach_c[1]`$.  
...
```{r}
broom::augment(mod2) %>% mutate(rn = 1:nrow(.)) %>% arrange(.resid) %>% tbl_df %>%
...
```{r}
# expected value of \beta_0j for each group, ignoring the random effects
pred_pop <- as.matrix(unique(data.frame(unclass(dat)) %>% mutate(I = 1) %>% select(I, gmc_texp, gmc_mean_pop_t))) %*% as.vector(fixef(mod2)[c(1,3,4)]) 

pred_pop <- pred_pop %>% data.frame() %>% setNames("B0") %>% mutate(class = as.character(1:100)) %>% full_join(ranef(mod2)[[1]]["popteach_c"] %>% mutate(class = rownames(.))) %>%
  arrange(desc(popteach_c))

ols2 <- lm(popular ~ 1 , data = dat %>% filter(class == pred_pop$class[1]))
...
# Now get the separate OLS regressions for each school.
lm_fit <- function(data) {
  fit <- lm(popular ~ 1, data)
  t(coef(fit)) %>% tbl_df %>% setNames(c("Intercept"))
}

By_School <- dat %>%
  group_by(class) %>%
  nest() %>%
  mutate(mod = map(data, lm_fit)) %>%
  unnest(mod, .drop = T)

# By_School <- ddply(HSB_Data, ~School, lm_fit)
# add to the Coef matrix and define as a dataframe.
Coef <- as.data.frame(cbind(Coef,By_School))
names(Coef) <- c("EB_Intercept","EB_Slope","Intercept_Var","Covariance","Covariance_Too",
                 "Slope_Variance","Intercept_Res","Slope_Res","School","OLS_Intercept") #,"OLS_Slope")
Coef$I_lwr <- Coef$EB_Intercept-sqrt(Coef$Intercept_Var)*1.96
Coef$I_upr <- Coef$EB_Intercept+sqrt(Coef$Intercept_Var)*1.96
Coef$S_lwr <- Coef$EB_Slope-sqrt(Coef$Slope_Var)*1.96
Coef$S_upr <- Coef$EB_Slope+sqrt(Coef$Slope_Var)*1.96
```

Class $`r pred_pop$class[1]`$ most exceeded their expected popularity of $`r round(pred_pop$B0[1],2)`$ by $`r round(pred_pop$popteach_c[1],2)`$, for total expected value of $`r pred_pop$B0[1] + pred_pop$popteach_c[1]`$. The simple (OLS) mean popularity of this class was $`r round(coef(ols2),2)`$. These two estimate are different because the MLM expected values leverage information about the relationship between mean classroom popularlity and teaching experience.  
...
```{r}
broom::augment(mod3) %>% mutate(student = rownames(.)) %>% tbl_df %>% arrange(.resid)

(dat %>% filter(row_number() %in% 1197))$popular
```
