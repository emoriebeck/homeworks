...
Coef <- bind_cols(Coef, ols_mods %>% unnest(ols, .drop = T)) %>% tbl_df %>%
  setNames(c("EB_Intercept","EB_Slope","Intercept_Var","Covariance","Covariance_Too",
                 "Slope_Var","Intercept_Res","Slope_Res","School","OLS_Intercept", "OLS_Slope")) %>%
...
Coef %>% select(EB_Intercept, EB_Slope) 

```

Class $`r ols_mods$class[1]`$ most exceeded their expected popularity by $`r round((L2_Resid %>% arrange(desc(popteach_c)))[1,2],2)`$. The simple (OLS) mean popularity of this class was $`r round(Coef$OLS_Intercept[1],2)`$ and the empirical Bayes estimate was $`r round(Coef$EB_Intercept[1],2)`$. These two estimate are different because the MLM expected values leverage information about the relationship between mean classroom popularlity and teaching experience.  
