...
res_fit2_df  %>%
  ...
  kable(., "latex", booktabs = T, escape = F,
        caption = "Q3b ghlt()", col.names = c("Term", "b", "CI"),
        align = c("l", "c", "c"))
```

There is a linear ($b_{linear}$ `r round((res_fit2_df %>% filter(term == ".L"))$b, 2)`, CI = [`r round((res_fit2_df %>% filter(term == ".L"))$lwr, 2)`, `r round((res_fit2_df %>% filter(term == ".L"))$upr, 2)`]) and quadratic ($b_{quadratic}$ `r round((res_fit2_df %>% filter(term == ".Q"))$b, 2)`, CI = [`r round((res_fit2_df %>% filter(term == ".Q"))$lwr, 2)`, `r round((res_fit2_df %>% filter(term == ".Q"))$upr, 2)`]) but not cubic ($b_{cubic}$ `r round((res_fit2_df %>% filter(term == ".C"))$b, 2)`, CI = [`r round((res_fit2_df %>% filter(term == ".C"))$lwr, 2)`, `r round((res_fit2_df %>% filter(term == ".C"))$upr, 2)`]). In other words, reading scores linearly increase with age, but also show a quadratic trend, such that rates of improvement slow over time.  
...
The plot suggests that age increased over time, such that the linear and quadratic trends evident in both models reflect changes in age of the sample. But as noted previously, the different scale (1,2,3,4) will influence the size of the slopes and the intercepts.  
