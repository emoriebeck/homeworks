...
\hspace{1em}TD1 & 2.55 & [2.42, 2.62]\\
\hspace{1em}TD2 & 4.09 & [4.01, 4.21]\\
\hspace{1em}TD3 & 5.09 & [5.02, 5.17]\\
\hspace{1em}TD4 & 5.80 & [5.64, 5.92]\\
\addlinespace[0.3em]
\multicolumn{3}{l}{\textbf{Random}}\\
\hspace{1em}$\tau_{00}$ & 0.00 & [0.00, 0.15]\\
$R^2_m$ & 0.55 & \\
$R^2_c$ & 0.55 & \\
\bottomrule
\end{tabular}
...
\NormalTok{res_fit2_df <-}\StringTok{ }\NormalTok{res_fit2}\OperatorTok{$}\NormalTok{confint }\OperatorTok{%>%}\StringTok{ }\KeywordTok{data.frame}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\hspace{1em}age\_10 &  &  & 0.56 & [0.55, 0.58] & 0.53 & [0.50, 0.55] & 0.50 & [0.48, 0.53]\\
\hspace{1em}age\_10\_2 &  &  &  &  & -0.05 & [-0.05, -0.04] & -0.05 & [-0.05, -0.04]\\
\hspace{1em}age\_10\_3 &  &  &  &  &  &  & 0.00 & [0.00, 0.01]\\
\hspace{1em}Intercept & 4.15 & [4.02, 4.30] & 4.43 & [4.35, 4.57] & 4.67 & [4.52, 4.77] & 4.66 & [4.63, 4.71]\\
$R^2_c$ & 0.11 &  & 0.87 &  & 0.91 &  & 0.91 & \\
$R^2_m$ & 0.00 &  & 0.58 &  & 0.60 &  & 0.60 & \\
\addlinespace[0.3em]
\multicolumn{9}{l}{\textbf{Random}}\\
\hspace{1em}$\tau_{00}$ & 0.29 & [0.15, 0.37] & 0.85 & [0.77, 1.00] & 1.12 & [0.99, 1.30] & 1.12 & [1.07, 1.28]\\
...
\includegraphics{Beck_HW_6_files/figure-latex/unnamed-chunk-7-1.pdf}
...
\addlinespace[0.3em]
\multicolumn{3}{l}{\textbf{Fixed}}\\
\hspace{1em}Intercept & 1.41 & [-0.01, 2.78]\\
\hspace{1em}age\_10 & 0.03 & [-0.25, 0.17]\\
\hspace{1em}momage & 0.13 & [0.07, 0.18]\\
\hspace{1em}age\_10\_2 & 0.01 & [-0.05, 0.07]\\
\hspace{1em}age\_10:momage & 0.02 & [0.01, 0.03]\\
\hspace{1em}momage:age\_10\_2 & -0.00 & [-0.00, 0.00]\\
\addlinespace[0.3em]
\multicolumn{3}{l}{\textbf{Random}}\\
\hspace{1em}$\tau_{00}$ & 1.07 & [0.94, 1.31]\\
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{means <-}\StringTok{ }\NormalTok{dat }\OperatorTok{%>%}\StringTok{ }\KeywordTok{summarize_at}\NormalTok{(}\KeywordTok{vars}\NormalTok{(momage), }\KeywordTok{funs}\NormalTok{(}\DataTypeTok{mean =} \KeywordTok{mean}\NormalTok{(., }\DataTypeTok{na.rm =}\NormalTok{ T), }\DataTypeTok{sd =} \KeywordTok{sd}\NormalTok{(., }\DataTypeTok{na.rm =}\NormalTok{ T)))}
...
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{pred =} \KeywordTok{predict}\NormalTok{(fit4c, }\DataTypeTok{newdata =}\NormalTok{ ., }\DataTypeTok{re.form =} \OtherTok{NA}\NormalTok{),}
...
\includegraphics{Beck_HW_6_files/figure-latex/unnamed-chunk-9-1.pdf}
