...
dat %>% filter(years == 5 & pubs == 5 & sex == 0 & Institution == "Tier 3" & outcome == "interview")

O1 <- exp(predict(fit_1, newdata = jobs %>% filter(ID == 124)))["H"]
O1 / (1 + O1)
@
...
<<>>=
# get cases that match this because I'm too lazy to  create a data frame
dat %>% filter(years == 6 & pubs == 7 & sex == 0 & Institution == "Tier 3" & outcome == "interview")

O2 <- exp(predict(fit_1, newdata = jobs %>% filter(ID == 152)))["H"]
@

The difference in odds will be \Sexpr{round(O1-O2, 2)} lower odds of being hired.
