...
\hlstd{dat} \hlopt{%>%} \hlkwd{filter}\hlstd{(years} \hlopt{==} \hlnum{5} \hlopt{&} \hlstd{pubs} \hlopt{==} \hlnum{5} \hlopt{&} \hlstd{sex} \hlopt{==} \hlnum{0} \hlopt{&} \hlstd{Institution} \hlopt{==} \hlstr{"Tier 3"} \hlopt{&} \hlstd{outcome} \hlopt{==} \hlstr{"interview"}\hlstd{)}
\end{alltt}
\begin{verbatim}
## # A tibble: 2 x 8
##      ID Institution   sex years  pubs outcome   pubs_c years_c
##   <int> <fct>       <dbl> <int> <int> <fct>      <dbl>   <dbl>
## 1   431 Tier 3          0     5     5 interview  0.698   -1.09
## 2   433 Tier 3          0     5     5 interview  0.698   -1.09
\end{verbatim}
\begin{alltt}
...
\begin{alltt}
\hlcom{# get cases that match this because I'm too lazy to  create a data frame}
\hlstd{dat} \hlopt{%>%} \hlkwd{filter}\hlstd{(years} \hlopt{==} \hlnum{6} \hlopt{&} \hlstd{pubs} \hlopt{==} \hlnum{7} \hlopt{&} \hlstd{sex} \hlopt{==} \hlnum{0} \hlopt{&} \hlstd{Institution} \hlopt{==} \hlstr{"Tier 3"} \hlopt{&} \hlstd{outcome} \hlopt{==} \hlstr{"interview"}\hlstd{)}
\end{alltt}
\begin{verbatim}
## # A tibble: 0 x 8
...
The difference in odds will be -0.7 lower odds of being hired.
