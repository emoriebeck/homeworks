...
The MSA for each item range from \Sexpr{round(min(KMO1$MSAi))} to \Sexpr{round(max(min(KMO1$MSAi)))}, with a mean of \Sexpr{round(mean(min(KMO1$MSAi)))}, indicating strong evidence for using a data reduction technique. 
...
The two models fit the data relatively well ($RMSEA_{unrotated} = \Sexpr{fa_1$RMSEA[1]}$, $RMSEA_{varimax} = \Sexpr{fa_2$RMSEA[1]}$, $RMSEA_{oblimin} = \Sexpr{fa_3$RMSEA[1]}$, $TLI_{unrotated} = \Sexpr{fa_1$TLI}$; $TLI_{varimax} = \Sexpr{fa_2$TLI}$; $TLI_{oblimin} = \Sexpr{fa_3$TLI}$). 
...
fa_1$Structure %>% unclass %>%
  ...
  mutate(Facet = str_remove_all(Facet, "_")) %>%
