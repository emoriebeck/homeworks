...
rownames(dat2) <- dat$ID #1:nrow(dat2)
(mv <- mvn(dat2,mvnTest="mardia", multivariatePlot="qq",multivariateOutlierMethod="quan",showOutliers=TRUE))
remove <- as.numeric(as.character(mv$multivariateOutliers$Observation[1]))
dat2 <- dat %>% filter(!(ID %in% remove)) 
@
...
<<>>=
(CB_1 <- cortest.bartlett(R=r,n=nrow(dat2)))
@
...
<<>>=
par(mfrow=c(1,2))
scree_1 <- fa.parallel(dat2 %>% select(-ID), fa="both")
...
<<>>=
par(mfrow = c(1,1))
vss_1 <- vss(dat2 %>% select(-ID), n = 25, rotate = "none", fm = "pc")
@
...
<<>>=
fa_1 <- fa(dat2 %>% select(-ID), nfactors = 5, rotate = "none", scores = T)
fa_2 <- fa(dat2 %>% select(-ID), nfactors = 5, rotate = "varimax", scores = T)
fa_3 <- fa(dat2 %>% select(-ID), nfactors = 5, rotate = "oblimin", scores = T)
...
<<>>=
cor(fa_1$scores) %>% round(.,2)

cor(fa_2$scores) %>% round(.,2)

cor(fa_3$scores) %>% round(.,2)
@
