...
idata <- tibble(measure = factor(c("Anxiety", "Persist", "Perform")))
manova2 <- car::Anova(m1, data = dat, idesign = ~measure, idata = idata, multivariate = T)
...
dat <- dat %>% 
  mutate(H1.1 = as.numeric(as.character(mapvalues(Group, 1:3, c(0,1,0)))),
         H2.1 = as.numeric(as.character(mapvalues(Group, 1:3, c(2,-1,-1)))),
