...
Checking the PCA suggests that there is a multivariate outlier in PCA 4 (Sentence_Completion)

But let's check multivariate normality
<<>>=
dat2 <- dat %>% select(-ID) %>% as.matrix
(mv <- mvn(dat,mvnTest="mardia", multivariatePlot="qq",multivariateOutlierMethod="quan",showOutliers=TRUE))
@
...
while(nrow(mv$multivariateOutliers) > 0){
  dat2 <- dat %>% filter(!(row_number() %in% remove)) %>% select(-ID)
  mv <- mvn(dat2, mvnTest="mardia", multivariatePlot="qq",multivariateOutlierMethod="quan",showOutliers=TRUE)
  mv$multivariateOutliers
  CV <- cov(dat2)
  D2_1 <- mahalanobis(dat2,center=colMeans(dat2),cov=CV)
  remove <- c(remove, which(D2_1 == max(D2_1)))
  print(sprintf("Case %s removed. This is the %s round", remove[k], k))
  k <- k + 1
}
@
...
<<>>=
CV <- cov(dat %>% select(-ID))
D2_1 <- mahalanobis(dat %>% select(-ID),center=colMeans(dat %>% select(-ID)),cov=CV)
remove <- which(D2_1 == max(D2_1))

dat <- dat %>% filter(row_number() != remove)
@

<<>>=
(mv <- mvn(dat %>% select(-ID),mvnTest="mardia", multivariatePlot="qq",multivariateOutlierMethod="quan",showOutliers=TRUE))
@

Now there are 14 outliers. Let's remove the next largest and keep going.  
<<>>=
CV <- cov(dat %>% select(-ID))
D2_1 <- mahalanobis(dat %>% select(-ID),center=colMeans(dat %>% select(-ID)),cov=CV)
remove <- which(D2_1 == max(D2_1))
