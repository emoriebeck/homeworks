...
\hlstd{nested.dat} \hlkwb{<-} \hlstd{nested.dat} \hlopt{%>%}
  ...
  \hlkwd{mutate}\hlstd{(}\hlkwc{mds} \hlstd{=} \hlkwd{map2}\hlstd{(d_mat, dim,} \hlopt{~}\hlkwd{smacofSym}\hlstd{(.x,} \hlkwc{ndim} \hlstd{= .y,}\hlkwc{verbose}\hlstd{=}\hlnum{FALSE}\hlstd{,}\hlkwc{type}\hlstd{=}\hlstr{"interval"}\hlstd{,}\hlkwc{itmax}\hlstd{=}\hlnum{1000}\hlstd{)),}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlstd{cdat} \hlkwb{<-} \hlstd{(nested.dat} \hlopt{%>%}
  ...
  \hlkwd{mutate}\hlstd{(}\hlkwc{Country} \hlstd{=} \hlkwd{unique}\hlstd{(dat}\hlopt{$}\hlstd{Country))}

\hlstd{dat.wide} \hlkwb{<-} \hlstd{dat} \hlopt{%>%}
  \hlkwd{select}\hlstd{(}\hlopt{-}\hlstd{z,}\hlopt{-}\hlstd{probit)} \hlopt{%>%}
  \hlkwd{spread}\hlstd{(}\hlkwc{key} \hlstd{= item,} \hlkwc{value} \hlstd{= raw)}

\hlstd{p1} \hlkwb{<-} \hlstd{cdat} \hlopt{%>%} \hlkwd{ggplot}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{x} \hlstd{= D1,} \hlkwc{y} \hlstd{= D2))} \hlopt{+}
  \hlkwd{geom_point}\hlstd{()} \hlopt{+}
  \hlkwd{geom_label}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{label} \hlstd{= Country))} \hlopt{+}
  \hlkwd{labs}\hlstd{(}\hlkwc{main} \hlstd{=} \hlstr{"D1 v D2"}\hlstd{)} \hlopt{+}
  \hlkwd{theme_classic}\hlstd{()}

\hlstd{p2} \hlkwb{<-} \hlstd{cdat} \hlopt{%>%} \hlkwd{ggplot}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{x} \hlstd{= D1,} \hlkwc{y} \hlstd{= D3))} \hlopt{+}
  \hlkwd{geom_point}\hlstd{()} \hlopt{+}
  \hlkwd{geom_label}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{label} \hlstd{= Country))} \hlopt{+}
  \hlkwd{labs}\hlstd{(}\hlkwc{main} \hlstd{=} \hlstr{"D1 v D3"}\hlstd{)} \hlopt{+}
  \hlkwd{theme_classic}\hlstd{()}

\hlstd{p3} \hlkwb{<-} \hlstd{cdat} \hlopt{%>%} \hlkwd{ggplot}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{x} \hlstd{= D2,} \hlkwc{y} \hlstd{= D3))} \hlopt{+}
  \hlkwd{geom_point}\hlstd{()} \hlopt{+}
  \hlkwd{geom_label}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{label} \hlstd{= Country))} \hlopt{+}
  \hlkwd{labs}\hlstd{(}\hlkwc{main} \hlstd{=} \hlstr{"D2 v D3"}\hlstd{)} \hlopt{+}
  \hlkwd{theme_classic}\hlstd{()}

\hlstd{gridExtra}\hlopt{::}\hlkwd{grid.arrange}\hlstd{(p1, p2, p3,} \hlkwc{nrow} \hlstd{=} \hlnum{2}\hlstd{)}
\end{alltt}
\end{kframe}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{cor}\hlstd{(cdat} \hlopt{%>%} \hlkwd{select}\hlstd{(}\hlopt{-}\hlstd{Country), dat.wide} \hlopt{%>%} \hlkwd{select}\hlstd{(}\hlopt{-}\hlstd{Country))} \hlopt{%>%}
  \hlkwd{round}\hlstd{(.,} \hlnum{2}\hlstd{)}
\end{alltt}
\begin{verbatim}
##    arrogant caring_ordinary charismatic dangerous intolerant strong_leader
## D1    -0.94            0.88        0.52     -0.92      -0.93          0.31
## D2    -0.29           -0.29       -0.81     -0.08      -0.19         -0.16
## D3    -0.01            0.10        0.17     -0.07      -0.19         -0.93
##    well_qualified
## D1           0.94
## D2          -0.02
## D3          -0.17
...
\hlstd{dat} \hlopt{%>%}
  \hlkwd{mutate}\hlstd{(}\hlkwc{Country} \hlstd{=} \hlkwd{str_remove_all}\hlstd{(Country,} \hlstr{" "}\hlstd{),}
         ...
         \hlstr{"Indonesia"}\hlstd{,} \hlstr{"Brazil"}\hlstd{,} \hlstr{"South Korea"}\hlstd{,} \hlstr{"Canada"}\hlstd{,} \hlstr{"Tanzania"}\hlstd{),} \hlstr{"Cluster"}\hlstd{,} \hlstr{"Other Countries"}\hlstd{))} \hlopt{%>%}
  \hlcom{# gather(key = item, value = value, -Country, -cluster) %>%}
  \hlstd{Rmisc}\hlopt{::}\hlkwd{summarySE}\hlstd{(.,} \hlkwc{measurevar} \hlstd{=} \hlstr{"raw"}\hlstd{,} \hlkwc{groupvars} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"item"}\hlstd{,} \hlstr{"cluster"}\hlstd{))} \hlopt{%>%}
  \hlkwd{mutate}\hlstd{(}\hlkwc{rank} \hlstd{=} \hlkwd{rank}\hlstd{(}\hlkwd{desc}\hlstd{(raw)))} \hlopt{%>%}
  \hlkwd{arrange}\hlstd{(rank)} \hlopt{%>%}
  \hlkwd{mutate}\hlstd{(}\hlkwc{item} \hlstd{=} \hlkwd{factor}\hlstd{(item,} \hlkwc{levels} \hlstd{=} \hlkwd{unique}\hlstd{(item)))} \hlopt{%>%}
  \hlkwd{ggplot}\hlstd{(}\hlkwd{aes}\hlstd{(}\hlkwc{x} \hlstd{= item,} \hlkwc{y} \hlstd{= raw,} \hlkwc{ymin} \hlstd{= raw} \hlopt{-} \hlstd{ci,} \hlkwc{ymax} \hlstd{= raw} \hlopt{+} \hlstd{ci,} \hlkwc{fill} \hlstd{= cluster))} \hlopt{+}
    \hlkwd{scale_y_continuous}\hlstd{(}\hlkwc{limits} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{0}\hlstd{,}\hlnum{100}\hlstd{),} \hlkwc{breaks} \hlstd{=} \hlkwd{seq}\hlstd{(}\hlnum{0}\hlstd{,}\hlnum{100}\hlstd{,}\hlnum{25}\hlstd{))} \hlopt{+}
    \hlkwd{scale_fill_manual}\hlstd{(}\hlkwc{values} \hlstd{=} \hlkwd{c}\hlstd{(}\hlstr{"red"}\hlstd{,} \hlstr{"white"}\hlstd{))} \hlopt{+}
...
\hlstd{mds_3_dat_p} \hlkwb{<-} \hlstd{(nested.dat} \hlopt{%>%}
  \hlkwd{filter}\hlstd{(dim} \hlopt{==} \hlnum{3} \hlopt{&} \hlstd{type} \hlopt{==} \hlstr{"probit"}\hlstd{))}\hlopt{$}\hlstd{mds[[}\hlnum{1}\hlstd{]]}\hlopt{$}\hlstd{conf} \hlopt{%>%}
  \hlstd{data.frame}
