...
fit1 <- cfa(b5.mod, dat2, orthogonal = T, missing = 'ML', std.lv = T)
...
<<>>=
fit2 <- cfa(b5.base, dat2, missing = 'ML', std.lv = T)
...
fit3 <- cfa(b5.mod, dat2, missing = 'ML', std.lv = T)
...
b5.mod <- paste(b5.lc, '\n\n', b5.uncorr, sep = '', collapse = '')
fit4 <- cfa(b5.mod, dat2, missing = 'ML', std.lv = T)
