...
coefs2 %>% mutate(id = factor(id, levels = orders$id)) %>%
  ggplot(aes(x = id, y = b)) +
    ...
    theme_classic() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1, size = rel(.6 )))
...
```{r, results='asis'}
fit5 <- lmer(rse ~ rse_lag_1 + posmn + negmn + 
    (rse_lag_1 + posmn + negmn | id), data = dat)
...
By partialling out the previous day's self esteem, we are able to look at the unique of current negative and positive emotion on the self-esteem.  

## Part B
Fit the model and comment on the magnitude and interpretation of the coefficients for positive and negative events.  

There is a negative effect of current negative events on self-esteem (b = `r (tab5 %>% filter(term == "negmn"))$b`; 95\% CI `r (tab5 %>% filter(term == "negmn"))$CI`). The current day's negative events predict lower self esteem.   

There is a posistive effect of current positive events on self-esteem (b = `r (tab5 %>% filter(term == "posmn"))$b`; 95\% CI `r (tab5 %>% filter(term == "posmn"))$CI`). The current day's positive predict higher self esteem.   
...
tab6 %>% select(-type) %>%
  ...
  group_rows("Fit", 13, 14) %>%
  group_rows("Random", 7, 12) %>%
  add_header_above(c(" " = 1, "Model Q6" = 2))
```
...
There is no effect of residual positive events on self-esteem (b = `r (tab6 %>% filter(term == "posmn\\_lag\\_1"))$b`; 95\% CI `r (tab6 %>% filter(term == "posmn\\_lag\\_1"))$CI`). The previous day's positive events do not predict self esteem.    
...
There is no effect of residual negative events on self-esteem (b = `r (tab6 %>% filter(term == "negmn\\_lag\\_1"))$b`; 95\% CI `r (tab6 %>% filter(term == "negmn\\_lag\\_1"))$b`). The previous day's negative events do not predict self esteem.   
...
fit7 <- lmer(rse ~ rse_lag_1*cesd_z + posmn*cesd_z + negmn*cesd_z + 
               posmn_lag_1*cesd_z + negmn_lag_1*cesd_z + 
    (posmn + negmn + rse_lag_1 + posmn_lag_1 + negmn_lag_1 | id), data = dat, control = cl2)
tab7 <- table_fun(fit7)
```
...
tab7 %>%
  select(-type) %>%
  kable(., "latex", escape = F, digits = 2, booktabs = T,
        col.names = c("", rep(c("b", "CI"), times = 1))) %>%
  kable_styling(full_width = F) %>%
  group_rows("Fixed", 1,12) %>%
  group_rows("Fit", 19, 20) %>%
  group_rows("Random", 13, 18) %>%
  add_header_above(c(" " = 1, "Full" = 2))  
```

There are two moderating effects of depression score: 1 SD change in depression is associated with a `r (tab7 %>% filter(term == "cesd\\_z:posmn\\_lag\\_1"))$b` * current posmn increase (95 \% CI `r (tab7 %>% filter(term == "cesd\\_z:posmn\\_lag\\_1"))$CI`) in self-esteem. In other words, current positive events are especially beneficial for those who are more depressed.  

In addition, a 1SD change in depression is a associated with a `r (tab7 %>% filter(term == "cesd\\_z:posmn\\_lag\\_1"))$b` * posmn\_lag\_1 increase (95 \% CI `r (tab7 %>% filter(term == "cesd\\_z:posmn\\_lag\\_1"))$CI`). In other words, past positive events only benefit the self-esteem of those who are less depressed than average.  
...
crossing(
  ...
  labs(title = "Lagged Positive Events") +
    geom_line() +
    theme_classic()

crossing(
  cesd_z = c(-1,0,1),
  posmn = seq(min(dat$posmn, na.rm = T), max(dat$posmn, na.rm = T),1),
  negmn = 0,
  posmn_lag_1 = 0,
  negmn_lag_1 = 0,
  rse_lag_1 = 0
  ...
  mutate(pred = predict(fit7, newdata = ., re.form = NA),
         cesd = mapvalues(cesd_z, c(-1, 0, 1), c("-1 SD", "0 SD", "+1 SD"))) %>%
  ggplot(aes(x = posmn, y = pred, color = cesd)) +
  labs(title = "Lagged Positive Events") +
    geom_line() +
    theme_classic()
```
