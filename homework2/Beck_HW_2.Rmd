...
Yes, match minority status predicts math achievement, $`r \gamma_{10} = (tab %>% filter(term == "minorityMinority"))$b`$, 95\% CI $= `r (tab %>% filter(term == "minorityMinority"))$CI`$.  

## Part B  
(b) What is the expected (mean) level of math achievement for non-minority students?  
The expected mean level of math achievement for non-minority students is $`r \gamma_{00} = (tab %>% filter(term == "(Intercept)"))$b`$, 95\% CI $= `r (tab %>% filter(term == "(Intercept)"))$CI`$.  
...
Yes, females have lower math achievement than males, controlling for SES and minority status, $\gamma_{20} = `r (tab2 %>% filter(term == "female"))$b`$, 95\% CI $`r(tab2 %>% filter(term == "female"))$CI`$.  
...
Controlling for gender and minority status, a one unit increase in SES is associated with a $`r (tab2 %>% filter(term == "GC.ses"))$b`$ (95\% CI $`r(tab2 %>% filter(term == "female"))$CI`$) increase in math achievement.  
## Part c  
...
## Part d  
(d) How much Level 1 variance is accounted for by this model compared to the fully unconditional model?   
`r round(sigma(mod2),2)`\% of the variance is accounted for by the fully conditional model.  
...
Sector predicts the level 1 intercepts, $\gamma01 = `r (tab3 %>% filter(term == "sector"))$b`$, bootstrapped 95\% CI $`r (tab3 %>% filter(term == "sector"))$CI`$. In other words, Catholic schools have higher average SES than public schools by $`r (tab3 %>% filter(term == "sector"))$b`$.  
...
Sector is also a significant predictor of the the level 1 slope for minority -- that is, SES varies as a function of both minority status and school sector, $\gamma01 = `r (tab3 %>% filter(term == "minorityMinority:sectorCatholic"))$b`$, bootstrapped 95\% CI $`r (tab3 %>% filter(term == "minorityMinority:sectorCatholic"))$CI`$. In other words, Minority students at Catholic high schools have SES that is on average $`r (tab3 %>% filter(term == "minorityMinority:sectorCatholic"))$b`$ than non-minority students at public high schools $`r (tab3 %>% filter(term == "(Intercept)"))$b`$.

## Part c 
Does sector significantly predict the Level 1 slope for ses ($\beta_{3j}$)?  If so, provide an interpretation of the relationship.  

Sector is also a significant predictor of the the level 1 slope for ses -- that is, SES varies as a function of both minority status and school sector, $\gamma01 = `r (tab3 %>% filter(term == "sectorCatholic:GC.ses"))$b`$, bootstrapped 95\% CI $`r (tab3 %>% filter(term == "sectorCatholic:GC.ses"))$CI`$. In other words, Minority students at Catholic high schools have SES that is on average $`r (tab3 %>% filter(term == "minorityMinority:sectorCatholic"))$b`$ than non-minority students at public high schools $`r (tab3 %>% filter(term == "(Intercept)"))$b`$.
