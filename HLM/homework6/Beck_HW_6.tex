...
\StringTok{  }\KeywordTok{kable}\NormalTok{(., }\StringTok{"latex"}\NormalTok{, }\DataTypeTok{booktabs =}\NormalTok{ T, }\DataTypeTok{escape =}\NormalTok{ F,}
        \DataTypeTok{caption =} \StringTok{"Q2b ghlt()"}\NormalTok{, }\DataTypeTok{col.names =} \KeywordTok{c}\NormalTok{(}\StringTok{"Term"}\NormalTok{, }\StringTok{"b"}\NormalTok{, }\StringTok{"CI"}\NormalTok{),}
...
Model 3: Level 1: \$ read\_\{ti\} = \pi\emph{\{0i\} +
\pi}\{1i\}(age-10)\emph{\{ti\} + \pi}\{2i\}(age-10)\^{}2\_\{ti\} +
e\_\{ti\}\$ Level 2: \$ \pi\emph{\{0i\} = \beta}\{00\} + r\_\{0i\}\$\\
\$ \pi\emph{\{1i\} = \beta}\{10\} + r\_\{1i\}\$\\
